...
df = xls.parse(sheet_name="流量_输入变量")  
...
lags = [1, 2, 3, 7, 14, 30]
for lag in lags:
    df[f"流量_lag{lag}"] = df["逐日平均流量（m3/s）"].shift(lag)

df["流量_MA3"] = df["逐日平均流量（m3/s）"].rolling(window=3).mean()
df["流量_MA7"] = df["逐日平均流量（m3/s）"].rolling(window=7).mean()
df["流量_MA14"] = df["逐日平均流量（m3/s）"].rolling(window=14).mean()
...
plt.xlabel("Date")
plt.ylabel("Daily Average Flow(m3/s)")
plt.title("Actual vs Predicted Daily Average Flow")
...
# 保存图表
output_path = "/Users/athena/Desktop/本科毕设/4_建立水质预测模型_清水铺_ANN"
