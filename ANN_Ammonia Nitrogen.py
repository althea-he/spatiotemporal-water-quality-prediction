...
df = xls.parse(sheet_name="氨氮_输入变量")  # 使用“氨氮”输入表
...
lags = [1, 2, 3, 7, 14, 30]
for lag in lags:
    df[f"氨氮_lag{lag}"] = df["氨氮(mg/L)"].shift(lag)

df["氨氮_MA3"] = df["氨氮(mg/L)"].rolling(window=3).mean()
df["氨氮_MA7"] = df["氨氮(mg/L)"].rolling(window=7).mean()
df["氨氮_MA14"] = df["氨氮(mg/L)"].rolling(window=14).mean()
...
plt.title("Actual vs Predicted Ammonia Nitrogen with Warnings")
...
# 保存图表
output_path = "/Users/athena/Desktop/本科毕设/4_建立水质预测模型_清水铺_ANN/Actual_vs_Predicted_Ammonia_Nitrogen_with_Warnings.png"
